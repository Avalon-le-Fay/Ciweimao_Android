...
class NativeHttpServer(
    ...
    override fun serve(session: IHTTPSession): Response {
        val prepared = try {
            HttpSupport.prepareJsonRequest(session)
        } catch (_: JSONException) {
            return HttpSupport.error("请求 JSON 无效", Response.Status.BAD_REQUEST)
                .apply { closeConnection(true) }
        } catch (error: IllegalArgumentException) {
            return HttpSupport.error(error.message ?: "请求体无效", Response.Status.BAD_REQUEST)
                .apply { closeConnection(true) }
        } catch (_: Exception) {
            return HttpSupport.error("请求体读取失败", Response.Status.BAD_REQUEST)
                .apply { closeConnection(true) }
        }
        return servePrepared(prepared)
    }

    private fun servePrepared(session: IHTTPSession): Response {
