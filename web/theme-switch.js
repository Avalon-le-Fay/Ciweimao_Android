...
function updateTheme() {
    ...
    try {
        if (window.CwmAndroid && typeof window.CwmAndroid.setSystemBarTheme === "function") {
            window.CwmAndroid.setSystemBarTheme(theme);
        }
    } catch (error) {
        // External browsers intentionally do not expose the native bridge.
    }
    const clearThemeChanging = () => root.classList.remove("theme-changing");
    if (document.readyState === "loading") {
        clearThemeChanging();
    } else {
        requestAnimationFrame(() => requestAnimationFrame(clearThemeChanging));
    }
}
